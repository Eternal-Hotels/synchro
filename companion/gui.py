...
class SynchroCompanionApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Synchro Companion")
    ...
    def run_in_background(self, busy_message, target):
        ...
        def worker():
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("Synchro Companion", str(error)))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Synchro Companion GUI and scheduled sync runner."
    )
    parser.add_argument(
        "--scheduled",
        action="store_true",
        help="Run a headless sync using the saved .env settings and then exit."
    )
