...
def hash_manifest_path():
    ...
    return runtime_dir() / "sync-hashes.json"
...
def run_verifone_commander_export(
    ...
    if not username:
        raise SystemExit("Missing Verifone Commander username. Fetch endpoint config or save it in Synchro Companion first.")
    if not password:
        raise SystemExit("Missing Verifone Commander password. Fetch endpoint config or save it in Synchro Companion first.")
